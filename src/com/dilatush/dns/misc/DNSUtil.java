...
public class DNSUtil {
    ...
    public static List<DNSResourceRecord> normalizeResourceRecords( final List<DNSResourceRecord> _rrs, final DNSRRType... _types ) {
        ...
        // get just the records of the type we want, and that haven't expired, with the new domain name...
        return filterResourceRecords( _rrs, _types ).stream()   // return a streamed list of unexpired records of the types we want...
                .map( rr -> rr.changeNameTo( odn ) )            // change the domain name in each record to the one orginally queried for...
                .collect( Collectors.toList());                 // collect a list of our results...
    }
